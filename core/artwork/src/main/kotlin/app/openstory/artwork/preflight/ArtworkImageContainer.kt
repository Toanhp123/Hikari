...
internal fun inspectArtworkImageContainer(file: File, declaredMediaType: String): EncodedImageBounds =
    RandomAccessFile(file, "r").use { input ->
        ...
        if (detected.mediaType != declaredMediaType) artworkFailure(ArtworkFailureReason.MEDIA_TYPE_REJECTED)
        when (detected.mediaType) {
            "image/png" -> if (containsPngChunk(input, "acTL")) artworkFailure(ArtworkFailureReason.DECODE_FAILED)
            "image/webp" -> if (isAnimatedWebp(input)) artworkFailure(ArtworkFailureReason.DECODE_FAILED)
        }
        detected
    }

internal fun validateArtworkDimensionLimits(width: Long, height: Long) {
...
private fun detectImageBounds(input: RandomAccessFile): EncodedImageBounds? {
    ...
    return when {
        ...
        header.copyOfRange(0, RIFF.size).contentEquals(RIFF) &&
            header.copyOfRange(WEBP_OFFSET, WEBP_OFFSET + WEBP.size).contentEquals(WEBP) -> readWebpBounds(input)
        else -> null
    }
}
...
private fun readWebpBounds(input: RandomAccessFile): EncodedImageBounds? {
    ...
    while (offset + WEBP_CHUNK_HEADER <= input.length()) {
        ...
        when (String(typeBytes, Charsets.US_ASCII)) {
            "VP8 " -> return validateWebpImageBounds(canvasBounds, readVp8Bounds(input, payloadOffset, length))
            "VP8L" -> return validateWebpImageBounds(canvasBounds, readVp8lBounds(input, payloadOffset, length))
            "VP8X" -> {
                if (canvasBounds != null) return null
                canvasBounds = readVp8xBounds(input, payloadOffset, length) ?: return null
                validateArtworkDimensionLimits(canvasBounds.width, canvasBounds.height)
            }
        }
...
private fun validateWebpImageBounds(
    ...
    if (imageBounds != null) validateArtworkDimensionLimits(imageBounds.width, imageBounds.height)
    return imageBounds?.takeIf { bounds ->
        canvasBounds == null || (canvasBounds.width == bounds.width && canvasBounds.height == bounds.height)
    }
}
