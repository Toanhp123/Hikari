    ...
    fun rank(items: List<CatalogEntryWithStory>): List<RankedCatalogStory> = items.groupBy { it.storyId }
        .map { (id, grouped) ->
            val contributions = grouped.map { item ->
                ...
                compareBy<CatalogRankContribution> { it.entry.pluginId.value }
                    .thenBy { it.entry.sourceId },
            )
            val scored = contributions.filter { it.normalizedScore != null }
            val weight = scored.sumOf { it.priorityWeight }
            val orderingScore = if (weight == 0.0) {
                0.0
            } else {
                scored.sumOf { it.normalizedScore!! * it.priorityWeight } / weight
            }
            RankedCatalogStory(id, orderingScore, contributions)
        }.sortedWith(
            compareByDescending<RankedCatalogStory> { it.orderingScore }
