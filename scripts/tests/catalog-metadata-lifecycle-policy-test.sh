...
story_readiness="$root/feature/catalog/src/main/kotlin/app/openstory/catalog/ui/story/StoryCanonicalReadiness.kt"
search="$root/catalog/src/main/kotlin/app/openstory/catalog/search/CatalogSearchService.kt"
fallback="$root/catalog/src/main/kotlin/app/openstory/catalog/details/CatalogFullMetadataFallbackService.kt"
fusion="$root/catalog/src/main/kotlin/app/openstory/catalog/fusion/CatalogFusionEngine.kt"
...
[[ -f "$fallback" ]] || fail "Story Full fallback service is missing"
[[ -f "$story_readiness" ]] || fail "Story canonical readiness owner is missing"
grep -q 'fullMetadata.requireFull(routeStoryId)' "$story_readiness" || fail "Story AUTO Full enrichment bypasses fallback service"
grep -q 'metadata.refresh(' "$story" || fail "Story explicit refresh must remain source-specific"
grep -q 'fusion.rankedEligibleSourceKeys' "$fallback" || fail "Full fallback duplicates source ranking instead of reusing Fusion ordering"
grep -q 'CatalogMetadataLevel.Full' "$fallback" || fail "Full fallback does not request Full metadata"
! grep -q 'CatalogFullMetadataFallbackService' "$search" || fail "Search must remain network-free from Story Full fallback"
! grep -q 'CatalogMetadataLevel.Full' "$search" || fail "Search production path must not request Full metadata"
grep -q 'fun rankedEligibleSourceKeys' "$fusion" || fail "Fusion does not expose the shared eligible-source ordering"
