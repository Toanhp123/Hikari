...
GRADLEW="$TEMP_DIR/fake-gradlew" \
  "$ROOT_DIR/scripts/verify-instrumentation.sh" 26 >/dev/null
...
GRADLEW="$TEMP_DIR/fake-gradlew" \
  "$ROOT_DIR/scripts/verify-instrumentation.sh" 37 >/dev/null 2>&1
STATUS=$?
set -e
...
echo "verify-instrumentation.sh contract verified."
