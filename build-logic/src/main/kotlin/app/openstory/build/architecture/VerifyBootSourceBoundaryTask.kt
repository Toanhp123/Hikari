...
abstract class VerifyBootSourceBoundaryTask : DefaultTask() {
    @get:InputFile
    ...
    @TaskAction
    fun verifyBoundary() {
        val buildScriptFile = appBuildScript.get().asFile.canonicalFile
        val appDirectory = buildScriptFile.parentFile
        ...
        val violations = BootSourceBoundaryVerifier.verify(
            sources = sources,
            buildScript = buildScriptFile.readText(),
            policy = policy,
        )

        if (violations.isNotEmpty()) {
            val report = buildString {
                appendLine("Foundation source boundary verification failed:")
        ...
        logger.lifecycle(
            "V2 boot source boundary verified for ${sources.size} production files.",
        )
    }
