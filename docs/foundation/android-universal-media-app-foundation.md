...
**Revision:** V1 Foundation R4.47 - First Slice Task 11 Closed / Task 12 Next
**Status:** Project Baseline / V1 First Slice In Progress
**Platform:** Android  
**Primary language:** Kotlin  
**UI direction:** Jetpack Compose  
**Last foundation review:** 2026-09-20 - Task 11 is CLOSED from user-owned device evidence: the repository PowerShell harness built/installed once, Phase A passed through real DocumentsUI/SAF + scan + Media3 checkpoint, the harness then performed external `adb shell am force-stop`, and Phase B passed in a fresh process with durable Library/source/progress reconstruction. Task 12 is next. Retained JDK-25 criteria do not establish canonical JDK-17 parity.
...
- Task 11 implementation: `ProcessDeathSafEndToEndTest` provides two separately invocable phases. Phase A drives the production MainActivity into the real `OpenDocumentTree` picker, waits for persisted read authority + WorkManager scan + one durable Library item, resolves the canonical source, exercises real Media3 playback, seeks/plays/pauses and waits for the immediate durable pause checkpoint. Test-only evidence in app-private files records only expected IDs/counts/checkpoint for Phase B. Phase B starts in a fresh instrumentation process after the repository harness performs external `adb shell am force-stop`; it verifies the persisted grant, same MediaId/source provenance, no scan required for Library reconstruction, resume within tolerance, and no duplicate Media/SourceBinding/Asset/Library rows. Production code, schema, permissions, module edges and navigation are unchanged.
- Task 11 harness: `scripts/task11-process-death.ps1` and `.sh` build/install once, clear state before Phase A only, stage the existing self-owned 30-second 640x360 MP4 under `Download/HikariTask11`, resolve the installed instrumentation component, run Phase A, externally force-stop `app.universalmedia.debug`, then run Phase B without reinstall/clear between phases. `scripts/tests/task11-process-death-harness-test.sh` statically guards phase ordering, fixture reuse, persisted-SAF/canonical-row assertions and the production evidence-reader prohibition.
- `PASS` - 2026-09-20 user-owned Task 11 device gate: `./scripts/task11-process-death.ps1` built the target and instrumentation APKs successfully, Phase A completed `OK (1 test)` in 8.584s, the repository harness then executed its external real process stop, Phase B completed `OK (1 test)` in 2.885s, and the script terminated with `Task 11 process-death / persisted-SAF device evidence: PASS`. This closes the actual OS process-death + persisted-SAF acceptance for the first local slice; no production bypass or same-process substitute was used.
...
- `PASS` - 2026-09-19 user confirmed all four Task 10 class-filtered commands in the active plan succeeded, including `verifyArchitecture`. Reviewed local connected-test XML: 31 tests, 0 failures/errors/skips on Redmi Note 9S / Android 15 (10 RoomMediaStoreTest, 9 SafLocalStorageTest, 4 LocalScanWorkerTest, 5 ScanSafetyTest, 1 ScanCompositionTest, 2 PlaybackRouteTest). Architecture PASS is based on user confirmation of the combined app/architecture command. Together with 19 focused JVM tests, these results close Task 10. No device gate was run by the agent; actual OS process-death/persisted-SAF acceptance was deferred to Task 11 and is now closed by the Task 11 evidence above.
...
- Task 9 development evidence: initial RED exposed missing orchestration, then a missing Back boundary. Two further regression tests reproduced duplicate START during the controller's IDLE propagation window and late queued playback after Back; both are now GREEN. The debug-only/non-exported Activity fixture hosts the production root UI in the target APK for actual Activity recreation. It uses a separate temporary Room database, real resolver, fake access port and recording playback port. No project dependency, public module API, production permission or schema change. Independent reviewer startup failed due to unavailable provider credentials; final review is author self-review only.
- `PASS` - 2026-09-19 user-confirmed Task 9 acceptance: `./gradlew.bat :app:connectedDebugAndroidTest "-Pandroid.testInstrumentationRunnerArguments.class=app.universalmedia.PlaybackRouteTest" --no-daemon` plus the handed-off real-MP4 app flow (Library selection, play/pause/seek, Activity recreation, Back/reopen with position retained and no duplicate card). Reviewed local XML: 2 tests, 0 failures/errors/skips, exit 0 on Redmi Note 9S / Android 15. Real Media3 playback/surface acceptance is user confirmation; the automated fixture uses a recording playback port. Together with 10 focused JVM tests, this closes Task 9. No device gate was run by the agent; actual OS process-death acceptance was deferred to Task 11 and is now closed by the Task 11 evidence above.

- Task 8 implementation policy: 5-second periodic sampling during active playback; unchanged positions do not write; pause/seek/stop/transition/ended/teardown checkpoint established playback. A 64-entry FIFO replaces cross-target conflation; writes select the accepted revision serially. Queue overflow, stale CAS, exceptions or 5-second sink timeout disable that run and expose a save failure. Teardown drains accepted events for at most 10 seconds while the process remains alive; neither teardown nor a blocked/failing store guarantees durability after OS kill.
- Task 8 domain policy: exact timestamp reuse requires the requested canonical target and same Asset/revision; binding changes alone do not invalidate equivalent representation provenance. Missing/changed provenance returns typed start-from-beginning without modifying durable state. App persistence preserves existing COMPLETED state on backward movement. Public policy/state contract changes require user-owned `verifyArchitecture`; no module edge, table/schema, permission or navigation change is introduced.
- Task 8 development evidence: policy RED failed on missing policy symbols. Initial GREEN attempt exposed two redundant instrumentation `!!` assertions under `-Werror`, corrected. Changed-file formatter required renaming the domain file to `VideoResumeDecision.kt`, corrected. Independent reviewer startup failed because the tool's inherited model was unavailable; final review is author self-review only.
- `PASS` - 2026-09-19 Task 8 focused gate: `./gradlew.bat :core:domain:test --tests '*VideoResumePolicyTest' :playback:media3:testDebugUnitTest --tests '*PlaybackCheckpointPolicyTest' --tests '*PlaybackCheckpointWriterTest' :app:testDebugUnitTest --tests '*PlaybackProgressAdapterTest' :data:compileDebugAndroidTestKotlin :playback:media3:compileDebugAndroidTestKotlin --no-daemon`, batched with changed-file `spotlessKotlinApply` using absolute-path `spotlessIdeHook`. BUILD SUCCESSFUL in 30s, exit 0. Reviewed XML: 13 tests, 0 failures/errors (3 resume, 4 policy, 4 writer, 2 adapter). Instrumentation compiled only. Retained JDK-25 criteria do not prove JDK-17 parity.
- `PASS` - 2026-09-19 user-confirmed Task 8 acceptance: `./gradlew.bat :playback:media3:connectedDebugAndroidTest "-Pandroid.testInstrumentationRunnerArguments.class=app.universalmedia.playback.media3.PlaybackServiceTest" verifyArchitecture --no-daemon` and `./gradlew.bat :data:connectedDebugAndroidTest "-Pandroid.testInstrumentationRunnerArguments.class=app.universalmedia.data.RoomMediaStoreTest#reopenRestoresIdentityLibraryAndBackwardSeekAndRejectsStaleCheckpoint" --no-daemon`. Reviewed local XML: 3 Media3 tests and 1 Room test, 0 failures/errors/skips on Redmi Note 9S / Android 15. Architecture PASS is based on user confirmation of the combined command. These results close Task 8; they prove checkpoint production and database reopen/resume compatibility, not OS process death. No device gate was run by the agent.
...
- `PASS` - 2026-09-19 user-confirmed Task 4 runtime and architecture checkpoint: `./gradlew.bat :ingestion:local:connectedDebugAndroidTest "-Pandroid.testInstrumentationRunnerArguments.class=app.universalmedia.ingestion.local.LocalScanWorkerTest" verifyArchitecture --no-daemon`. Reviewed local XML: 4 tests, 0 failures/errors/skips on Redmi Note 9S / Android 15. Architecture PASS is based on the user's confirmation of this exact combined command.
- `PASS` - 2026-09-19 user-confirmed app composition acceptance: `./gradlew.bat :app:connectedDebugAndroidTest "-Pandroid.testInstrumentationRunnerArguments.class=app.universalmedia.ScanCompositionTest" --no-daemon`. Reviewed local XML: 1 test, 0 failures/errors/skips on the same device. These results close Task 4. Worker recreation uses a reused scheduler ID with fresh domain runs; actual process-death acceptance remains the later slice gate.
...
Implement **Task 12 - Security, Performance and Resource-Lifetime Audit** from the active plan. Task 11 is closed from real process-death/persisted-SAF evidence; do not reopen or broaden it unless Task 12/13 produces contradiction evidence.

Tasks 0-11 are CLOSED. Task 11 has user-owned Phase A + external force-stop + fresh-process Phase B evidence PASS. Retain the user-requested JDK-25 daemon file; no canonical JDK-17 parity or baseline migration is claimed.
...
TASK 7 PLAYBACK API / MEDIA3 SESSION SERVICE - CLOSED
        |
TASK 8 TYPED VIDEO PROGRESS / RESUME COMPATIBILITY - CLOSED
        |
...
TASK 10 IDEMPOTENCY / RESCAN / FAILURE-SAFETY CHARACTERIZATION - CLOSED
        |
TASK 11 REAL PROCESS-DEATH / PERSISTED-SAF EVIDENCE - CLOSED
        |
NEXT - TASK 12 SECURITY / PERFORMANCE / RESOURCE-LIFETIME AUDIT
```
...
The active plan is `docs/superpowers/plans/2026-09-18-first-local-vertical-slice.md`. New sessions must read the Current Control Block first, then that plan, and resume at **Task 12**. Bootstrap and Tasks 0-11 are closed with executable evidence. Macrobenchmark measurements remain a separate later device-performance gate.
