...
> **Phase-5 acceptance checkpoint (2026-08-22):** Tasks 33–35 are **VERIFIED / CLOSED**. The accepted review workflow has one durable command/result service, one queue/navigation surface, and one contextual Story surface over the same `ReconciliationCase` / revision. Queue ranking is presentation-only; canonical/local projections provide titles/covers without hidden Detail/network repair; `KEEP_SEPARATE` changes durable status; `DEFER` leaves the case `PENDING` and advances only contextual suppression; invariant/domain-state conflicts remain non-destructive; and protected mapping conflicts use only domain-provided candidates before resubmitting the same Task-33 merge command. Contextual Story review uses canonical Story identity and hands exact-case protected conflicts to the shared review destination. Developer-checkout acceptance includes green app unit/Android-test compilation, 10/10 app navigation connected tests, 11/11 focused Room merge-coordinator connected tests, a final 6/6 Review ViewModel run after correcting the test-only Java-21 `List.removeFirst()`/Java-17 runtime mismatch, successful feature Catalog compilation, and final canonical `./scripts/verify.sh` after behavior-preserving UI orchestration and Detekt cleanup without suppressions. No Room migration was added; schema 9 remains current. Phase 6 starts at Task 36; Task 37 still owns operation-level Full fallback.

> **Phase-6 Task-36 acceptance checkpoint (2026-08-22):** Task 36 is **VERIFIED / CLOSED** while Phase 6 remains open. Runtime routing now converges on one `CanonicalEngineOrchestrator`: committed Home/Details/Search evidence emits narrow facts, identity+Fusion changes reconcile before resolving the current canonical owner and rebuilding Fusion once, Story source-preference changes use the same boundary, explicit Story Full refresh no longer double-rebuilds Fusion, and user-approved merge notifies post-merge orchestration only after authoritative Room success. Repository adapters remain fact/persistence boundaries; conditional `POST_MERGE_DERIVED` remains atomic inside the Room merge transaction. Developer-checkout acceptance includes green focused Catalog/feature unit gates, 47/47 selected Room connected tests on Redmi Note 9S - 15, and a final green canonical `./scripts/verify.sh`; module boundaries remain 14 production + 1 android-test and Room schema 9 is unchanged. The accepted evidence is `docs/internal/checkpoints/canonical-catalog-reconciliation-fusion-phase-6-task-36.md`. Task 37 is now active and still owns operation-level Full fallback; Task 36 does not introduce a transactional outbox/recovery migration.

> **Phase-6 acceptance checkpoint (2026-08-22):** Tasks 36–38 are **VERIFIED / CLOSED**. Task 37 adds one Story AUTO Full fallback boundary that reuses Fusion's effective-primary/eligible-source ordering, falls through only on operation failure/unavailability, stops on any successful Full result even when optional fields are sparse, keeps explicit source refresh source-specific, and re-resolves identity after successful Full persistence. Task 38 preserves Task-36 routing and Room-owned atomic post-merge work while adding read-only schema-9 merge-lineage recovery for post-merge hard contradictions; candidate-index state refreshes before correction detection, and historical A/B cases reopen as `REVIEW + INVARIANT_BLOCKED` without auto-detach, split, or reversal. Developer-checkout acceptance includes green focused Catalog and feature unit gates, 13/13 selected Task-38 Room connected tests on Redmi Note 9S - 15, and a final green canonical `./scripts/verify.sh` after behavior-preserving constructor/test-fixture/Detekt cleanup with no new suppression. Room remains schema 9. Evidence is `docs/internal/checkpoints/canonical-catalog-reconciliation-fusion-phase-6.md`. Phase 7 starts at Task 39; the narrow commit-to-post-commit-callback process-death window remains explicit later recovery/outbox work rather than a Phase-6 migration.

> **Phase-7 Task-39 acceptance checkpoint (2026-08-22):** Task 39 is **VERIFIED / CLOSED**. Durable engine-work draining and policy/invariant safety passes reuse the schema-9 work queue and existing owners without adding a scheduler to domain modules. Background safety stays bounded, idempotent, redirect-aware, and app-composed. Evidence is `docs/internal/checkpoints/canonical-catalog-reconciliation-fusion-phase-7-task-39.md`.
