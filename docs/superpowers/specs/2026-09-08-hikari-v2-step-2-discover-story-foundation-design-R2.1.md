...
Date: 2026-09-08
Status: **REVISED — READY FOR IMPLEMENTATION-PLAN REVIEW; NOT YET IMPLEMENTATION-APPROVED**
Branch baseline: `v2/foundation-clean-boot`
Step 1 runtime/source baseline: `eb4d3bfd869a5b9df78a5802de31510b3984c3c3`
Revision: **R2.1 — red-team load-bearing corrections integrated (2026-09-08)**
...
Until the revised design and its implementation plan are separately approved, `docs/implementation/current-roadmap.md` remains correct in saying that no Step 2 production implementation is approved.
...
Step 2 enables both approved **Manga | Light Novel** media selections as first-class local presentation/read keys. This explicitly supersedes the earlier delivery restriction that left Light Novel disabled. Changing media type replaces only the selected bounded Discover observation/bootstrap scope; it does not trigger arbitrary Story Detail calls or execute acquisition directly from UI.
...
The exact visual language is redesignable. Step 2 is not pixel-compatible with V1.
...
- source identity/provenance required by the product contract.
...
- scroll position is retained where feasible;
...
The Step 1 root currently includes seven reviewed modules. Step 2 intentionally adds exactly four production modules: `:catalog:domain`, `:catalog:storage`, `:catalog:runtime`, and `:feature:catalog`. Architecture verification should assert the exact admitted set/edges rather than relying only on a loose module-count ceiling.
...
### 6.5 Why no separate image module yet
...
### 6.7 Step 1 architecture ratchet evolution
...
### 6.8 Step 2 build-surface admission
...
### 6.9 Build-variant and fixture-source admission
...
Discover uses one `LazyColumn`-style vertical scroll owner. Section internals may scroll horizontally when useful, but nested vertical grids/lists are rejected.
...
The exact visual form is not constitutional. Section semantics, stable identity, accessibility, and bounded cost are.
...
When content exists, manual refresh does not replace it with full-screen loading. Existing content stays visible and a small refresh indicator/issue surface communicates progress/failure.
...
Step 2 may expose user-initiated Discover refresh to exercise the real acquisition/import/failure path.
...
Step 2 has explicit user retry and bootstrap single-flight only. There is no exponential background retry engine, durable failure queue, or automatic repeated plugin execution.
...
9. The Step 1 build-surface ratchet is evolved, not deleted: exactly four new Step 2 **production** modules/approved edges are admitted and architecture checks remain fail-closed.
...
11. `:app` has exactly one reviewed Step 2 product edge (`:feature:catalog`) and no direct storage/runtime/image/network ownership.
12. `:catalog:model` and `:catalog:engine` remain quarantine/reference with no Step 2 runtime edge.
13. Production package dependency verification reports zero new package SCCs inside the Step 2 production modules.
14. Discover renders multiple semantic sections and supports both media selections as bounded persisted-data scopes.
15. The default first snapshot is bounded to the approved section caps; no per-card DB query or detail request is needed to render it.
16. Storage/read state distinguishes `Absent` from `Published(generation, cards)`; `Published(empty)` is a valid durable result.
17. A successful zero-card publication survives DB reopen/process recreation as `Published(empty)` and does not trigger automatic bootstrap again.
18. Only `Absent` may auto-bootstrap, and only when that build wiring has an admitted acquisition source; `Absent + no source` remains explicit.
19. `CatalogSourceKey` is host-authoritative and distinct from old canonical-engine source concepts.
20. `StoryId` uses the frozen `source-story:v1:<full SHA-256>` derivation from exact length-prefixed UTF-8 `CatalogSourceKey + sourceStoryId` bytes and does not change with mutable Story metadata.
21. Identity collisions fail closed; there is no numeric suffix/random/insertion-order collision repair.
22. Story selection routes with explicit source identity (`StorySourceRef`/equivalent), not implicit global source inference.
23. Story selection opens metadata-only Story Detail; Chapters/Reader are absent.
24. Story Detail read cost is keyed, has a fixed query-count upper bound, and uses bounded child/result cardinality independent of total Catalog size.
25. Acquisition identifiers/text/child collections/locators are validated against explicit bounds before persistence/UI expansion.
26. `CatalogSourceKey`, source version, and acquisition timestamp are host-stamped/verified provenance; untrusted payload data cannot impersonate another source or asset policy.
27. Persisted Discover/Story Detail provenance survives later source-version changes accurately.
28. Discover persistence uses one coherent materialized published-generation state observation; UI cannot observe half-replaced/mismatched generation/card snapshots.
29. Storage enforces duplicate `(sectionKind, itemPosition)` and `(sectionKind, storyId)` invariants inside one generation, using fixed section semantics + `item_position` without a redundant mutable `section_position` authority.
30. Discover publication accepts a zero-card generation atomically and leaves the prior state intact on failed publication.
31. Discover publication/pruning work scales with bounded previous/current snapshots, bounded removed delta, and bounded orphan overflow—not total historical Catalog rows.
32. The refresh critical path performs no global Story/detail scan or history-wide unbounded sort/count to maintain the 64-item orphan-retention policy.
33. Detail enrichment publishes summary/detail/children/provenance atomically and does not mutate the published Discover-card snapshot as a side effect.
34. Failed Story Detail publication preserves the complete prior usable detail state; observers do not see mixed old/new child collections.
35. Active Story pin registration/removal and Discover pruning obey one short mutation-gate ordering so a Story opened during refresh cannot be pruned in the race window.
36. The mutation gate is never held across source/network I/O, image work, long UI work, or user interaction.
37. Access-aging writes do not create reactive invalidation loops.
38. Real cover images render in Discover and Story Detail in local deterministic fixtures and the final plugin integration probe.
39. Cover acquisition/rendering can progress independently of rich Story Detail readiness.
40. Discover → Story Detail → Back uses stable asset identity and bounded memory/disk cache continuity without bitmap navigation copies.
41. Process recreation reconstructs Story Detail from saved narrow route identity + persistence, re-registers pins safely, and recovers source asset policy from `CatalogSourceKey`; it does not require retained DTO/bitmap state.
42. Card/image geometry remains stable while cover bytes arrive.
43. Presentation code does not own raw plugin URLs, source-host trust, redirect validation, or a generic HTTP client.
44. Trusted local cover locators persist stable logical asset IDs/versions, not raw Android resource integers.
45. Remote cover locators are typed/source-scoped; HTTPS host/redirect/timeout/media-type/encoded-size validation is enforced in remote integration.
46. Remote cover revision does not generically discard URI query data; a source-specific stable artwork token is used only when its reviewed contract proves equivalence.
47. Image memory/disk/concurrency have explicit ceilings; encoded response is ≤8 MiB; source width/height is ≤8,192 px; source pixel surface is ≤32,000,000 pixels; original-resolution decode for bounded UI display is rejected.
48. Manual refresh, if exposed, has one foreground owner and retains cached content on failure.
49. No process-start Catalog, Room, image, plugin, worker, scheduler, or network initializer is introduced.
50. The main/release manifest does not gain production `INTERNET` solely for the isolated Step 2 plugin gate.
51. Persistent/runtime/image state is bounded under repeated refresh, navigation, and aged datasets.
52. Deterministic returning Discover performs no acquisition/network work for both `Published(content)` and `Published(empty)`.
53. Memory-hit cover path performs no disk/network fetch; disk-hit cover path performs no network fetch.
54. Database/source/network/large-decode/importer work does not intentionally execute on Main.
55. A dedicated performance-validation task measures and may simplify the first multi-section implementation when evidence requires it.
56. Before final startup comparison, the Step 2 baseline/startup profile is regenerated from the final deterministic returning Discover/Story journey and its artifact/hash is recorded.
57. Startup and meaningful-content deltas are recorded against the accepted Step 1 baseline under equivalent profile/benchmark methodology.
58. A same-device TTID median regression >10% is optimized or explicitly reviewed rather than silently accepted.
59. Frame/jank behavior has an explicit benchmark review threshold and cannot be waived solely because TTID remains fast.
60. Repeated navigation/activation does not create monotonic growth in observers, active-work maps, Story pins, orphan-retention ownership, or decoded asset ownership beyond configured bounds.
61. The final local path passes architecture, variant, package-SCC, storage, UI, image, lifecycle, input-bound, concurrency-race, and performance gates before plugin integration begins.
62. The deterministic real-plugin gate uses the reviewed MangaUpdates reference behavior by default and proves all three Step 2 semantic sections without provider-title/ID guessing.
63. The real plugin executes in `:feature:catalog` `androidTest`/integration-only wiring against controlled deterministic transport and feeds the unchanged import/persistence/read/UI path.
64. Reference-plugin/harness provenance/hash is recorded; the harness adds no production app startup/runtime dependency on the full plugin system.
65. Live external plugin/network testing, if retained, is optional smoke evidence only and is not an acceptance/performance dependency.
66. Local/plugin acquisition failures or bounded-input rejections do not erase prior usable published content.
67. No placeholder, unresolved ownership rule, or deferred-decision marker remains for a load-bearing Step 2 design decision before implementation planning begins.
...
J. performance/scaling validation + aged-history bounded-work proof + evidence-driven UI/query simplification
K. regenerate Step 2 baseline/startup profile and run final deterministic benchmark comparison
L. isolated MangaUpdates real-plugin androidTest gate with controlled transport + provenance evidence
M. full Step 2 acceptance/freeze
```
...
- No global design system/DI/navigation migration is required to deliver the vertical slice.
...
Potential contradiction: deterministic Story identity was called “frozen later” while also being load-bearing. Resolution: R2.1 freezes the exact domain-separated, length-prefixed full SHA-256 v1 algorithm and fail-closed collision policy in the design itself.
...
Potential contradiction: deterministic benchmark seed is required while release must contain no seed and the app already has custom benchmark build types. Resolution: R2.1 freezes an explicit variant/source-set matrix and requires compatible Android-library variants plus release-artifact leakage tests.
...
This **R2.1** design is **ready for implementation-plan review**, but it is not itself permission to implement Step 2 production code.

The next artifact is a detailed Step 2 implementation plan. The plan must:

- preserve every normative decision in this design rather than reopening them by convenience;
- begin with exact build-surface/module/variant/package-SCC admission rather than temporarily disabling Step 1 ratchets;
- include the companion authoritative product-design update that enables Light Novel;
- encode the frozen `CatalogSourceKey + sourceStoryId -> StoryId v1` algorithm as golden-vector tests and implementation;
- encode `Absent` versus durable `Published(empty)` before UI/bootstrap work depends on it;
- encode atomic Story Detail publication, bounded orphan-retention/delta pruning, and the pin/prune mutation-gate race contract as focused tests;
- preserve host-owned provenance/asset-policy authority;
- convert the frozen acquisition/image/cache/concurrency ceilings into executable validation/configuration;
- implement the explicit debug/benchmark/profile/release/androidTest source matrix and prove release artifact cleanliness;
- include exact architecture, variant, package-SCC, storage, runtime, UI, image, concurrency, and focused test commands;
- include a dedicated self-review at each task boundary;
- place deterministic performance/scaling validation before real-plugin integration;
- regenerate the Step 2 baseline/startup profile only after the local path is finalized, then run the final same-device benchmark comparison;
- use the reviewed MangaUpdates reference behavior by default for the deterministic `androidTest` plugin gate and record exact provenance/hash of imported/executed plugin/support code;
- keep any live external plugin/network test optional and outside deterministic acceptance/performance numbers;
- stop before implementation until the implementation plan itself is reviewed and approved.

No Step 2 production code should be implemented merely because this revised design file exists.
