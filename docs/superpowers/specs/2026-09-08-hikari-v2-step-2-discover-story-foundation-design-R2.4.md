...
Revision: **R2.4 — Task 13 presentation-foundation / Task 14 visual-restoration split (2026-09-10)**
...
- one domain-neutral segmented single-choice control for Manga/Light Novel selection;
...
- source identity/provenance required for routing, persistence, authority, and integration evidence. Raw `CatalogSourceKey`, `sourceVersion`, acquisition timestamps, host-policy values, and similar internal authority fields are **not** user-facing labels in Step 2. A visible friendly source name requires an explicit user-safe presentation contract and must not be invented from internal provenance values.
...
### 6.5 `:core:designsystem` — minimal Step 2 slice

`:core:designsystem` is a presentation-only Android/Compose library with zero production project dependencies. It owns only the visual vocabulary with a proven Step 2 caller: root `HikariTheme`; stable top-level reviewed light/dark palette, typography, and Material3 shape values; a public immutable `HikariSpacing` singleton; `HikariSegmentedControl`; `HikariSectionHeader`; `HikariSkeleton`; `HikariEmptyState`; `HikariErrorState`; `HikariInlineFeedback`; and `HikariPullToRefresh`. It does not introduce a second public dimensions/shapes/type hierarchy when Material3 plus `HikariSpacing` already express the required contract. Theme values are selected, not rebuilt, during recomposition; `HikariTheme.kt` does not construct color schemes/typography/shapes or install a fixed-token `CompositionLocalProvider`. Step 2 uses platform/Compose typography only and performs no runtime/downloadable-font or Context/resource lookup.

The Step 2 visual token baseline is frozen rather than left to implementation taste. It salvages V1's base Material3 Coral/Teal/Warm-surface palette, base Serif-display/Sans-body typography values, and 8/12/20/28/36.dp Material3 shape family. The only palette deviations are root `background = Color.White` in light mode and `background = Color.Black` in dark mode so `HikariBootSurface` continues to match the accepted Step 1 `android:windowBackground`; V1 Cream/DarkBackground remain available only as surface/container roles. V1 semantic typography/colors/brushes, opacity, dimensions, breakpoints, layout ratios/policy, glyph geometry, motion policy, and all associated fixed-token `CompositionLocal`s are explicitly not admitted. Exact role/value code is frozen in Task 13 of the implementation plan and is the execution source of truth for this Step 2 admission.

It must not own feature copy, `DiscoverUiState`/`StoryDetailUiState`, retry/acquisition scheduling, `CoverLocator`, `CoverAssetKey`, image loading/cache, source trust, persistence, navigation state, Catalog domain semantics, data observation, lifecycle collection, coroutines, effects, service lookup, application-lifetime mutable state, or any vertical/horizontal scroll owner. Shared state/feedback primitives are caller-sized and do not force `fillMaxSize`. The segmented control is bounded to 2..5 options and validates size/selection/key uniqueness directly over the bounded input without building temporary key collections during composition. Production build files for this Step 2 slice do not admit Coil, `coil-network-okhttp`, OkHttp/raw HTTP, backdrop/blur/glass, Room, WorkManager, JavaScriptEngine, or plugin/runtime dependencies. Step 2 does not admit Robolectric/Roborazzi as part of this module's verification strategy.

The V1 module is reference evidence only. Task 13 salvages/adapts individual domain-neutral primitives; copying the V1 module/build file wholesale is rejected. The Big Update Structural Simplification audit is also normative here: every public primitive admitted by Task 13 must have a production caller in the Step 2 slice and must encode a stable semantic/visual rule, not merely reduce a few lines of Compose syntax. Zero-caller, test-only, or future-facing public Design System surfaces are removed before benchmark; small local UI duplication is preferred over a generic parameter-heavy base component when no stable shared rule exists.
...
The final Step 2 app presentation used for screenshot and performance acceptance consumes the minimal admitted `:core:designsystem` slice from the application root. `HikariStartupApp` installs `HikariTheme` once; Unknown, FirstRun, Discover, and Story consume it without nesting another theme at `CatalogEntryPoint`. The existing `Ready + firstFrameReached` composition gate remains the only trigger for Catalog entry. Repeated Hikari visual language (Material3 palette/typography/shapes, shared spacing, media segmented selection, section heading, static skeleton, empty/error/inline feedback, pull refresh) comes from the shared module. The root shape family reuses the reviewed V1 base values (8/12/20/28/36.dp) without re-admitting V1 semantic-shape locals; generic card/state/skeleton corner roles consume those Material3 shapes instead of recreating equivalent `RoundedCornerShape` objects feature-locally. Repeated generic spacing that exactly matches the admitted Hikari scale consumes `MaterialTheme.hikariSpacing`. Bounded feature-specific cover sizes/aspect ratios, odd geometry, section semantics, copy, state, and image ownership remain in `:feature:catalog`. Step 2 does not mechanically move every one-off `dp` value into shared tokens or add wrapper-for-wrapper abstractions merely to satisfy visual purity before benchmark.
...
The exact visual form is not constitutional. Section semantics, stable identity, accessibility, and bounded cost are. Manga and Light Novel availability is frozen Step 2 presentation policy, so one top-level literal immutable selector-option list lives at the presentation boundary rather than inside `DiscoverUiState`; it is not derived from `CatalogMediaType.entries.map` or rebuilt from state/composition. The shared segmented primitive performs direct bounded validation rather than `map`/`distinct`/set-building on every composition; ViewModel state carries only selected media plus content state. The bounded section list is emitted directly into the existing `LazyListScope`; Step 2 does not materialize a second flattened `DiscoverViewportRow` list/model solely to adapt section data to Compose.
...
- Story Detail presents cover/title/rating/status/content type as a coherent summary and groups rich metadata for scanning;
...
Task 14 prefers feature-local composables such as Discover-specific hero/card/ranked-row or Story-specific hero/metadata sections over new generic Design System wrappers. It does not add blur/backdrop/glass/shimmer/infinite animation, another scroll owner, eager full-feed prefetch, N+1 detail acquisition, or new image/runtime ownership merely to imitate V1 aesthetics.

Visual acceptance is an explicit human-reviewed evidence gate. Deterministic screenshots must be compared against the reviewed V1 reference set on hierarchy, artwork presentation, content density, typography/spacing coherence, metadata scanability, and adaptive layout. A screenshot test that executes successfully but records an obviously weaker UI is not a pass. Any deliberate regression requires an evidence-backed product/performance rationale in the Step 2 checkpoint.
...
- Task 14 deterministic screenshots are compared against the reviewed V1 reference set with an explicit human PASS/FAIL ledger for artwork prominence, hierarchy, density, typography/spacing, metadata scanability, light/dark coherence, and adaptive layout; successful capture alone is not visual acceptance.
...
14. Root Unknown/FirstRun/Catalog surfaces share one `HikariTheme`; Discover renders multiple semantic sections and both media selections as bounded persisted-data scopes using the admitted segmented/section/skeleton/empty/error/feedback vocabulary rather than feature-local theme/component forks.
15. After Task 13 foundation acceptance, Task 14 produces feature-local Discover and Story Detail composition that is human-reviewed as at least comparable to the reviewed V1 visual quality for artwork prominence, hierarchy, content density, typography/spacing coherence, metadata scanability, light/dark treatment, portrait-cover geometry, and compact/wider-screen reflow; pixel parity and V1 architecture transplantation are explicitly not required.
...
24. Story selection opens metadata-only Story Detail; Chapters/Reader are absent.
...
43. Card/image geometry remains stable while cover bytes arrive; initial loading uses static non-shimmer shared skeleton presentation and does not replace retained content during refresh.
...
68. No placeholder, unresolved ownership rule, or deferred-decision marker remains for a load-bearing Step 2 design decision before implementation planning begins; visual composition is intentionally assigned to Task 14 rather than ambiguously split with Task 13.
...
Potential contradiction: deterministic Story identity was called “frozen later” while also being load-bearing. Resolution: R2.4 freezes the exact domain-separated, length-prefixed full SHA-256 v1 algorithm and fail-closed collision policy in the design itself.
...
Potential contradiction: deterministic benchmark seed is required while release must contain no seed and the app already has custom benchmark build types. Resolution: R2.4 freezes an explicit variant/source-set matrix and requires compatible Android-library variants plus release-artifact leakage tests.
...
This **R2.4** design is **ready for implementation-plan review**, but it is not itself permission to implement Step 2 production code.
