...
**Execution status (2026-08-22): `VERIFIED — PHASE 5 CLOSED`.** Tasks 33–35 are accepted together as the durable review boundary. Task 33 owns review commands/results and the single merge-service path; Task 34 owns the queue/navigation projection over durable cases; Task 35 owns contextual Story prompting over that same case/revision and hands protected mapping conflicts to the shared review route. No Task-36 evidence orchestration or Task-37 Full fallback was pulled into this phase. Room remains schema 9. Acceptance evidence and the Java-17 test-fixture root cause are recorded in `docs/internal/checkpoints/canonical-catalog-reconciliation-fusion-phase-5.md`. **Task 36 is the active next task.**
...
**Execution status (2026-08-22): `VERIFIED — TASK 34 CLOSED`.** The accepted queue is a local/canonical projection over `ReconciliationCaseRepository.observePending()` plus read-only canonical titles/covers and user-state footprint; ranking is presentation-only and deterministic; actions delegate to Task 33; protected mapping choices are restricted to candidates returned by the domain result; non-resolvable domain conflicts remain explanatory/pending; and load failure is visible. `AppRoute.ReconciliationReview(caseId: String? = null)` supports both the queue and exact-case handoff, while `Review duplicates` is a utility-sheet destination rather than a top-level tab. No Details/network fallback is used to repair missing queue metadata.
...
**Execution status (2026-08-22): `VERIFIED — TASK 35 CLOSED`.** Story observes durable reconciliation state by canonical `StoryId`, projects `caseId + caseRevision`, applies a feature-owned contextual confidence threshold, respects the persisted suppression deadline, and keeps low-confidence cases queue-only. `DEFER` suppresses only contextual prompting and can reappear after expiry while the Story remains open; `KEEP_SEPARATE` disappears through durable case status; invariant-blocked cases are explanatory with no Merge action; protected mapping conflicts navigate to the shared exact-case review flow instead of duplicating resolution state. The final Story screen keeps orchestration focused by hosting prompt layout in `StoryReconciliationPrompt.kt`; no Task-36/37 behavior was added.
...
**Implementation amendment — 2026-08-22 (`VERIFIED — TASK 39 CLOSED`):** The Phase-6 tree exposed several constraints that the original Task-39 sketch did not encode precisely enough. This amendment supersedes the file/interface/scheduling details below where they conflict, while preserving the Task-39 product behavior and keeping Room at schema 9. The durable `canonical_engine_work` table is the retry source of truth; WorkManager is only the host wake/drain mechanism. Queue mutations use exact persisted snapshots so stale foreground/background completion or retry cannot delete newer work, and runnable `next_attempt_at_epoch_millis` also acts as a monotonic schema-free queue revision for otherwise-identical dirty events. Parked invariant work uses `Long.MAX_VALUE` and is never reopened by normal dirty coalescing; repairable policy-version parks are requeued only by bounded safety logic after the running binary can support them again. Rekeying during merge preserves parked failures, unions post-merge derived requirements, and fails closed as `canonical.maintenance.coalesced_invariant` when two different parked invariant classes coalesce.

Task 39 adds the Android-free `CanonicalEngineMaintenanceReader`, `CatalogReconciliationMaintenance`, `CanonicalMaintenanceHealthMarker`, and `PostMergeDerivedWorkDispatcher` boundaries because the maintenance service needs bounded persistence facts and cross-domain derived scheduling without moving SQL/WorkManager semantics into `:catalog`. `POST_MERGE_DERIVED` reasons now durably encode the exact required operations while retaining backward parsing for the legacy `story-merge-derived-state` reason. Local chapter reaggregation runs before any network work; when both are required, Mapping is chained before Chapter Sync. Foreground Fusion marks an exact durable snapshot before rebuilding, completes only that snapshot on semantic success, and wakes the drain if completion is stale or post-merge transaction-owned work may remain. Reconciliation's durable `ReevaluationScheduled` result also wakes the drain.

The original `ExistingWorkPolicy.KEEP`/`OpenStoryApplication.onCreate()` sketch is intentionally not used. A plain `KEEP` drain has a lost-wakeup window while the current worker is still RUNNING, and this repository's cold-start policy forbids scheduling work from `Application.onCreate()`. The app adapter therefore uses one serialized `canonical-engine-drain` chain with `APPEND_OR_REPLACE` plus an in-process coalescing gate, and defers the initial drain/daily-safety registration until after the first activity frame. Future queue timestamps schedule timestamp-keyed `KEEP` retry-wake workers; durable queue timestamps, not WorkManager backoff, remain authoritative. WorkManager `Result.retry()` is reserved for an outer infrastructure failure before the queue state can be durably classified. Daily safety stays periodic and returns success even when semantic invariants are found so one parked Story cannot poison the periodic chain.

Policy compatibility is fail-closed. Work requiring a policy version newer than the running binary, or persisted canonical/reconciliation state carrying a future policy version, is parked and the Story is marked degraded before older code can mutate it. The bounded daily safety pass requeues only those policy parks once supported again; a semantically unchanged Fusion rebuild repairs stale `DEGRADED` state health from the active generation. Safety queries are bounded to stale policy Stories, current pending reconciliation cases, recoverable parked policy work, and redirect inconsistencies (missing target, redirect chain, or self-redirect); there is no all-pairs Story scan.

The accepted patch scope is broader than the original file list only where required by these invariants: `RoomCanonicalEngineWorkRepository` owns race-safe queue semantics; `CanonicalEngineMaintenanceDao`/`RoomCanonicalEngineMaintenanceReader` own bounded read facts; `RoomStoryMergeWriter`/`RoomStoryMergeApplier` preserve the same atomic schema-9 merge-work contract under the stronger coalescing rules; `ChapterReaggregationService` supplies the local derived repair boundary; app DI and WorkManager adapters remain in `:app`. No migration, schema export, automatic reversal, new truth store, or Task-40/41 behavior is introduced.

**Developer verification status — 2026-08-22: `VERIFIED — TASK 39 CLOSED`.** The final Task-39 tree passes the focused Catalog maintenance/orchestration/Fusion/Reconciliation unit gate, the app worker/derived-dispatcher unit gate, and 17/17 selected Room connected tests (`RoomCanonicalEngineStateTest` + `RoomCanonicalEngineMaintenanceReaderTest`) on Redmi Note 9S - 15. The canonical `./scripts/verify.sh` then passes end-to-end, including canonical-engine policy, package/source-layout/architecture gates, Detekt, module boundaries, and Android lint; the run reports `BUILD SUCCESSFUL` with 628 actionable tasks and confirms that the Room schema export remained stable at schema 9. Acceptance details and the verification-fix history are recorded in `docs/internal/checkpoints/canonical-catalog-reconciliation-fusion-phase-7-task-39.md`. **Task 40 is the active next task.**
...
**Implementation amendment — 2026-08-22: `VERIFIED — TASK 40 CLOSED`.** Task 40 stays on Room schema 9 and uses the schema-9 merge/reversal audit foundation rather than adding a migration or full database snapshots. The implementation separates `StoryMergeReversalPlanner` assessment from execution, binds both to one Room coordinator, and extends the reverse request with the exact correction case id/revision when reversal originates from Task-38 contradictory-evidence review. That case revision is revalidated and the correction case is resolved to user-owned `RESOLVED_SEPARATE` inside the same Room transaction as the graph split, so process death cannot commit the split while leaving the review pending. The transaction resurrects the retired Story, restores provable source/Library/mapping/chapter/sync/progress ownership, removes only the exact forward redirect, detaches both active generations, increments preference/identity revisions monotonically, inserts one reversal audit row, and atomically marks both Stories dirty for Fusion and Reconciliation reevaluation. Repeated execution of an already-reversed merge is deterministic/idempotent.

Reversal safety is intentionally narrower than comparing the complete `postMergeAuthoritativeFingerprint`. Task 38 is expected to change identity evidence and therefore survivor identity revision/fingerprints; using the whole post-merge fingerprint as the reversal gate would make a valid correction impossible to reverse. Task 40 instead permits identity-evidence changes while explicitly proving that restorable/user-owned state has not drifted: exact historical source membership, deterministic merged source preference, Library state, mapping/rejection state, chapter/release side partition plus overrides/sync state, and reading progress. New/unowned sources, protected mapping changes, historical protected-target conflicts whose user-selected merge resolution was not recorded in payload v1, Library edits, cross-side chapter reassociation, unsafe progress changes, degraded/parked canonical state, unsupported future policy, nested redirect lineage, stale identity/case revision, or an audit already requiring review all fail closed with `REQUIRES_REVIEW_TO_REVERSE` and perform no writes. The complete post-merge fingerprint remains audit evidence, not the sole reverse permission check. Correction cases are hidden from the contextual Story prompt (which has no reversal affordance) and are handled by the full Review Queue, where only `Reverse safely`/`Later` are presented for post-merge corrections; `Keep separate` is not presented as a fake undo.
